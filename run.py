...
def run(
    ...
    for frame in range(1, frames + 1):
        ...
        next_state, reward, done, _ = envs.step(
            action
        )  # returns np.stack(obs), np.stack(action) ...
        ...
        if done.any():
            ...
            print(
                "\rEpisode {}\tFrame {} \tAverage100 Score: {:.2f}".format(
                    i_episode * worker, frame * worker, np.mean(scores_window)
                ),
                end="",
            )
            if i_episode % 100 == 0:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=10000000,
        help="Number of frames to train, default = 10 mio",
    )
    parser.add_argument(
        "-eval_every",
        type=int,
        default=250000,
        help="Evaluate every x frames, default = 250000",
    )
    parser.add_argument(
        "-eval_runs", type=int, default=2, help="Number of evaluation runs, default = 2"
    )
    parser.add_argument(
        "-seed",
        type=int,
        default=1,
        help="Random seed to replicate training runs, default = 1",
    )
    parser.add_argument(
        "-N", type=int, default=8, help="Number of Quantiles, default = 8"
    )
    parser.add_argument(
        "-munchausen",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=512,
        help="Size of the hidden layer, default=512",
    )
    parser.add_argument(
        "-n_step", type=int, default=1, help="Multistep IQN, default = 1"
    )
    parser.add_argument(
        "-m",
        "--memory_size",
        type=int,
        default=int(1e5),
        help="Replay memory size, default = 1e5",
    )
    parser.add_argument(
        "-lr", type=float, default=0.00025, help="Learning rate, default = 2.5e-4"
    )
    parser.add_argument(
        "-g",
        "--gamma",
    ...
    parser.add_argument(
        ...
        type=float,
        default=1e-3,
        help="Soft update parameter tau, default = 1e-3",
    )
    parser.add_argument(
        "-eps_frames",
        type=int,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.01,
        help="Final epsilon greedy value, default = 0.01",
    )
